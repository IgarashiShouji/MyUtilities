...
private:
    ...
    std::string str;
public:
    inline ClsValue(void)            : type(type_none),   str("") { data.data     = 0;   }
    inline ClsValue(std::string & s) : type(type_str) ,   str(s)  { data.data     = 0;   }
    inline ClsValue(const char * s)  : type(type_str) ,   str(s)  { data.data     = 0;   }
    inline ClsValue(char val)        : type(type_char),   str("") { data.ch       = val; }
    inline ClsValue(uint8_t val)     : type(type_uint8),  str("") { data.uint8    = val; }
    inline ClsValue(int8_t val)      : type(type_int8),   str("") { data.int8     = val; }
    inline ClsValue(uint16_t val)    : type(type_uint16), str("") { data.uint16   = val; }
    inline ClsValue(int16_t val)     : type(type_int16),  str("") { data.int16    = val; }
    inline ClsValue(uint32_t val)    : type(type_uint32), str("") { data.uint32   = val; }
    inline ClsValue(int32_t val)     : type(type_int32),  str("") { data.int32    = val; }
    inline ClsValue(uint64_t val)    : type(type_uint64), str("") { data.uint64   = val; }
    inline ClsValue(int64_t val)     : type(type_int64),  str("") { data.int64    = val; }
    inline ClsValue(float val)       : type(type_float),  str("") { data.v_float  = val; }
    inline ClsValue(double val)      : type(type_double), str("") { data.v_double = val; }

    inline ClsValue & operator = (ClsValue & src)  { type = src.type; data = src.data; str = src.str;   return *this; }
    inline ClsValue & operator = (std::string & s) { type = type_str;    str           = s;             return *this; }
    inline ClsValue & operator = (const char * s)  { type = type_str;    str           = s;             return *this; }
    ...
    inline size_t set_data_big(Type_v type, const uint8_t * buff, size_t size=0)
    {
        ...
        switch(type)
        {
        case type_uint64:   case type_int64:    case type_double: size=8; data.buff[0] = buff[7]; data.buff[1] = buff[6]; data.buff[2] = buff[5]; data.buff[3] = buff[4]; data.buff[4] = buff[3]; data.buff[5] = buff[2]; data.buff[6] = buff[1]; data.buff[7] = buff[0]; break;
        case type_uint32:   case type_int32:    case type_float:  size=4; data.buff[0] = buff[3]; data.buff[1] = buff[2]; data.buff[2] = buff[1]; data.buff[3] = buff[0]; break;
        case type_uint16:   case type_int16:                      size=2; data.buff[0] = buff[1]; data.buff[1] = buff[0]; break;
        case type_uint8:    case type_int8:     case type_char:   size=1; data.buff[0] = buff[0]; break;
        case type_str:
            {
                std::vector<char> buff_str(size+1);
                for(size_t idx=0, top=size-1; idx<size; idx++){ buff_str[idx] = buff[top-idx]; };
                buff_str[size] = '\0';
                str = &(buff_str[0]);
            }
            break;
    ...
    inline size_t get_data(uint8_t * dst, size_t sz=0) const
    {
        ...
        if(type == type_str){ std::memcpy(dst, str.c_str(), sz); }
        else                { std::memcpy(dst, data.buff,   sz); }
        return sz;
    }
    inline size_t get_data_big(uint8_t * dst, size_t sz=0)
    {
        size_t len = size();
        sz = (sz==0?len:(len<sz?len:sz));
        if(type != type_str){ const uint8_t * src = &(data.buff[0]); for(size_t idx=0, top=sz-1; idx<sz; idx++){ dst[top-idx] = src[idx]; } }
        else                { const char *    src = str.c_str();     for(size_t idx=0, top=sz-1; idx<sz; idx++){ dst[top-idx] = src[idx]; } }
        return sz;
    }
    ...
    inline uint64_t uint64(void)
    {
        ...
        case type_str:      return static_cast<uint64_t>(stoi(str));
        case type_none:
        default:
            break;
        }
    ...
    inline int64_t int64(void)
    {
        ...
        case type_str:      return static_cast<int64_t>(stoi(str));
        case type_none:
        default:
            break;
        }
    ...
    inline uint32_t uint32(void)
    {
        ...
        case type_str:      return static_cast<uint32_t>(stoi(str));
        case type_none:
        default:
            break;
        }
    ...
    inline int32_t int32(void)
    {
        ...
        case type_str:      return static_cast<int32_t>(stoi(str));
        case type_none:
        default:
            break;
        }
    ...
    inline uint16_t uint16(void)
    {
        ...
        case type_str:      return static_cast<uint16_t>(stoi(str));
        case type_none:
        default:
            break;
        }
    ...
    inline int16_t int16(void)
    {
        ...
        case type_str:      return static_cast<int16_t>(stoi(str));
        case type_none:
        default:
            break;
        }
    ...
    inline uint8_t uint8(void)
    {
        ...
        case type_str:      return static_cast<uint8_t>(stoi(str));
        case type_none:
        default:
            break;
        }
    ...
    inline int8_t int8(void)
    {
        ...
        case type_str:      return static_cast<int8_t>(stoi(str));
        case type_none:
        default:
            break;
        }
    ...
    inline char ch(void)
    {
        ...
        case type_str:      return static_cast<char>(stoi(str));
        case type_none:
        default:
            break;
        }
    ...
    inline double v_double(void)
    {
        ...
        case type_str:      return static_cast<double>(stod(str));
        case type_none:
        default:
            break;
        }
    ...
    inline float v_float(void)
    {
        ...
        case type_str:      return static_cast<float>(stof(str));
        case type_none:
        default:
            break;
        }
    ...
    inline float value(void){ return v_float(); }
    inline ClsValue::Type_v get_type(void) const { return type; }
    inline ClsValue::vData  get(void)      const { return data; }
    inline std::string      get_str(void)  const { return str;  }
};
//inline ClsValue::ClsValue(ClsValue::Type_v type, const uint8_t * buff, size_t size=0) { set_data(type, buff, size); }
...
public:
    ...
    inline void swap(BinaryControl & src)
    {
        size_t temp = pos;
        pos = src.pos;
        src.pos = temp;
    ...
    inline size_t fill(uint8_t val, size_t len)  { return memset(0, val, len); }
    inline size_t fill(uint8_t val=0)            { return memset(0, val, size()); }

    inline size_t get_data(uint8_t * dst, size_t len, size_t address=0) const
    {
        len = (len<size()?len:size());
        std::memcpy(dst, data(address), len);
        return len;
    }
    ...
    inline size_t memcpy(const uint8_t * src, size_t len) { return memcpy(0, src, len); }
    ...
    inline size_t memcpy(BinaryControl & src, size_t len) { return memcpy(0, 0, src, len); }
    inline size_t memcpy(BinaryControl & src)             { return memcpy(0, 0, src, (size()<src.size()?size():src.size())); }
    inline BinaryControl & operator += (BinaryControl & src) { resize(size() + src.size()); memcpy(size(), src.data(), src.size()); return *this; }
    inline uint8_t operator [] (size_t idx) { return (idx<bin.size() ? bin[idx]: 0); } const

    inline int32_t memcmp(size_t address_dst, size_t address_src, BinaryControl & src, size_t len) const
    {
        auto dst_len = size()     - address_dst;
        auto src_len = src.size() - address_src;
        if(dst_len < len) { len = dst_len; }
    ...
    inline int32_t memcmp(BinaryControl & src, size_t len) const { return memcmp(0, 0, src, len); }
    inline int32_t memcmp(BinaryControl & src) const { return memcmp(src, (size()<src.size()?size():src.size())); }
    ...
    inline std::list<size_t> toAddressList(std::string & addr)
    {
        uint16_t v_temp = 0;
